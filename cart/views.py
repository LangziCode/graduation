...
class CartUpdate(APIView):
    def post(self, request):
        ...
        discountprice=data['discountprice']
        id = data['id']
        Cart.objects.filter(id=id).update(
        id=id,
        tablename=tablename,
        userid=userid,
        goodid=goodid,
...
class CartDelete(APIView):
    def post(self, request):
        data = request.data
        for id in data:
            Cart.objects.filter(id=int(id)).delete()
...
class CartInfo(APIView):
    def get(self, request, id):
        id = id
        entity = Cart.objects.filter(id=int(id)).all()
        entityser = CartSer(entity, many=True)
...
class CartList(APIView):
    def get(self, request):
        ...
        if page is None or limit is None:
            return CartListNotPage(request)
        else:
            return CartListPage(request)
...
def CartListPage(request):
    ...
    queryset = Cart.objects.filter(q_objects).all()
    count = Cart.objects.filter(q_objects).count()
    ...
    result = CartSer(items, many=True)
    return Response({'code': 0, 'data': {'list': result.data, 'total': count}})

def CartListNotPage(request):
    userid = request.query_params.get('userid')
    query_params = {
                        'id__icontains': request.query_params.get('id'),
                                'addtime__icontains': request.query_params.get('addtime'),
    ...
    queryset = Cart.objects.filter(q_objects).all()
    count = Cart.objects.filter(q_objects).count()
    result = CartSer(queryset, many=True)
    return Response({'code': 0, 'data': {'list': result.data, 'total': count}})
