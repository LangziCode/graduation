...
class MessagessUpdate(APIView):
    def post(self, request):
        ...
        rpicture=data['rpicture']
        id = data['id']
        Messagess.objects.filter(id=id).update(
        id=id,
        userid=userid,
        username=username,
        content=content,
...
# 删除
class MessagessDelete(APIView):
    def post(self, request):
        data = request.data
        for id in data:
            Messagess.objects.filter(id=int(id)).delete()
        return Response({'code': 0, 'msg': '删除成功'})

# 查询详情
class MessagessInfo(APIView):
    def get(self, request, id):
        id = id
        entity = Messagess.objects.filter(id=int(id)).all()
        entityser = MessagessSer(entity, many=True)
        return Response({'code': 0, 'data': entityser.data[0]})

# 分页
class MessagessList(APIView):
    def get(self, request):
        page = request.query_params.get('page')  # 第几页
        limit = request.query_params.get('limit')  # 数量

        if page is None or limit is None:
            return MessagessListNotPage(request)
        else:
            return MessagessListPage(request)
...
def MessagessListPage(request):
    ...
    query_params = {
                        'id__icontains': request.query_params.get('id'),
                                ...
                                'cpicture__icontains': request.query_params.get('cpicture'),
                                'reply__icontains': request.query_params.get('reply'),
                                'rpicture__icontains': request.query_params.get('rpicture'),
                    }
    q_objects = Q()
    ...
    queryset = Messagess.objects.filter(q_objects).all()
    count = Messagess.objects.filter(q_objects).count()
    ...
    result = MessagessSer(items, many=True)
    return Response({'code': 0, 'data': {'list': result.data, 'total': count}})

def MessagessListNotPage(request):
    userid = request.query_params.get('userid')
    query_params = {
                        'id__icontains': request.query_params.get('id'),
                                'addtime__icontains': request.query_params.get('addtime'),
    ...
    queryset = Messagess.objects.filter(q_objects).all()
    count = Messagess.objects.filter(q_objects).count()
    result = MessagessSer(queryset, many=True)
    return Response({'code': 0, 'data': {'list': result.data, 'total': count}})

# 分页
class MessagessPage(APIView):
    def get(self, request):
        page = request.query_params.get('page')  # 第几页
        limit = request.query_params.get('limit')  # 数量
        sort = request.query_params.get('sort') # 排序
