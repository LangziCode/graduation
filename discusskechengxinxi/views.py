...
# 保存
class DiscusskechengxinxiSave(APIView):
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            reply = None
        Discusskechengxinxi.objects.create(
            refid=refid,
            userid=userid,
            nickname=nickname,
            content=content,
...
# 修改
class DiscusskechengxinxiUpdate(APIView):
    def post(self, request):
        ...
        reply=data['reply']
        id = data['id']
        Discusskechengxinxi.objects.filter(id=id).update(
        id=id,
        refid=refid,
        userid=userid,
        nickname=nickname,
...
# 删除
class DiscusskechengxinxiDelete(APIView):
    def post(self, request):
        data = request.data
        for id in data:
            Discusskechengxinxi.objects.filter(id=int(id)).delete()
        return Response({'code': 0, 'msg': '删除成功'})

# 查询详情
class DiscusskechengxinxiInfo(APIView):
    def get(self, request, id):
        id = id
        entity = Discusskechengxinxi.objects.filter(id=int(id)).all()
        entityser = DiscusskechengxinxiSer(entity, many=True)
        return Response({'code': 0, 'data': entityser.data[0]})

# 分页
class DiscusskechengxinxiList(APIView):
    def get(self, request):
        page = request.query_params.get('page')  # 第几页
        limit = request.query_params.get('limit')  # 数量

        if page is None or limit is None:
            return DiscusskechengxinxiListNotPage(request)
        else:
            return DiscusskechengxinxiListPage(request)
...
def DiscusskechengxinxiListPage(request):
    ...
    queryset = Discusskechengxinxi.objects.filter(q_objects).all()
    count = Discusskechengxinxi.objects.filter(q_objects).count()
    ...
    result = DiscusskechengxinxiSer(items, many=True)
    return Response({'code': 0, 'data': {'list': result.data, 'total': count}})

def DiscusskechengxinxiListNotPage(request):
    userid = request.query_params.get('userid')
    query_params = {
                        'id__icontains': request.query_params.get('id'),
                                'addtime__icontains': request.query_params.get('addtime'),
    ...
    queryset = Discusskechengxinxi.objects.filter(q_objects).all()
    count = Discusskechengxinxi.objects.filter(q_objects).count()
    result = DiscusskechengxinxiSer(queryset, many=True)
    return Response({'code': 0, 'data': {'list': result.data, 'total': count}})

# 分页
class DiscusskechengxinxiPage(APIView):
    def get(self, request):
        page = request.query_params.get('page')  # 第几页
        limit = request.query_params.get('limit')  # 数量
        sort = request.query_params.get('sort') # 排序
