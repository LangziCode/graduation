...
class ExampaperUpdate(APIView):
    def post(self, request):
        ...
        status=data['status']
        id = data['id']
        Exampaper.objects.filter(id=id).update(
        id=id,
        name=name,
        time=time,
        status=status,
...
# 删除
class ExampaperDelete(APIView):
    def post(self, request):
        data = request.data
        for id in data:
            Exampaper.objects.filter(id=int(id)).delete()
        return Response({'code': 0, 'msg': '删除成功'})

# 查询详情
class ExampaperInfo(APIView):
    def get(self, request, id):
        id = id
        entity = Exampaper.objects.filter(id=int(id)).all()
        entityser = ExampaperSer(entity, many=True)
        return Response({'code': 0, 'data': entityser.data[0]})

# 分页
class ExampaperList(APIView):
    def get(self, request):
        page = request.query_params.get('page')  # 第几页
        limit = request.query_params.get('limit')  # 数量

        if page is None or limit is None:
            return ExampaperListNotPage(request)
        else:
            return ExampaperListPage(request)
...
def ExampaperListPage(request):
    ...
    query_params = {
                        'id__icontains': request.query_params.get('id'),
                                ...
                                'time__icontains': request.query_params.get('time'),
    ...
    queryset = Exampaper.objects.filter(q_objects).all()
    count = Exampaper.objects.filter(q_objects).count()
    ...
    result = ExampaperSer(items, many=True)
    return Response({'code': 0, 'data': {'list': result.data, 'total': count}})

def ExampaperListNotPage(request):
    userid = request.query_params.get('userid')
    query_params = {
                        'id__icontains': request.query_params.get('id'),
                                'addtime__icontains': request.query_params.get('addtime'),
    ...
    queryset = Exampaper.objects.filter(q_objects).all()
    count = Exampaper.objects.filter(q_objects).count()
    result = ExampaperSer(queryset, many=True)
    return Response({'code': 0, 'data': {'list': result.data, 'total': count}})

# 分页
class ExampaperPage(APIView):
    def get(self, request):
        page = request.query_params.get('page')  # 第几页
        limit = request.query_params.get('limit')  # 数量
        sort = request.query_params.get('sort') # 排序
