...
# 保存
class DiscusskechengziyuanxinxiSave(APIView):
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            reply = None
        Discusskechengziyuanxinxi.objects.create(
            refid=refid,
            userid=userid,
            nickname=nickname,
            content=content,
...
# 修改
class DiscusskechengziyuanxinxiUpdate(APIView):
    def post(self, request):
        ...
        reply=data['reply']
        id = data['id']
        Discusskechengziyuanxinxi.objects.filter(id=id).update(
        id=id,
        refid=refid,
        userid=userid,
        nickname=nickname,
...
# 删除
class DiscusskechengziyuanxinxiDelete(APIView):
    def post(self, request):
        data = request.data
        for id in data:
            Discusskechengziyuanxinxi.objects.filter(id=int(id)).delete()
        return Response({'code': 0, 'msg': '删除成功'})

# 查询详情
class DiscusskechengziyuanxinxiInfo(APIView):
    def get(self, request, id):
        id = id
        entity = Discusskechengziyuanxinxi.objects.filter(id=int(id)).all()
        entityser = DiscusskechengziyuanxinxiSer(entity, many=True)
        return Response({'code': 0, 'data': entityser.data[0]})

# 分页
class DiscusskechengziyuanxinxiList(APIView):
    def get(self, request):
        page = request.query_params.get('page')  # 第几页
        limit = request.query_params.get('limit')  # 数量

        if page is None or limit is None:
            return DiscusskechengziyuanxinxiListNotPage(request)
        else:
            return DiscusskechengziyuanxinxiListPage(request)
...
def DiscusskechengziyuanxinxiListPage(request):
    ...
    queryset = Discusskechengziyuanxinxi.objects.filter(q_objects).all()
    count = Discusskechengziyuanxinxi.objects.filter(q_objects).count()
    ...
    result = DiscusskechengziyuanxinxiSer(items, many=True)
    return Response({'code': 0, 'data': {'list': result.data, 'total': count}})

def DiscusskechengziyuanxinxiListNotPage(request):
    userid = request.query_params.get('userid')
    query_params = {
                        'id__icontains': request.query_params.get('id'),
                                'addtime__icontains': request.query_params.get('addtime'),
    ...
    queryset = Discusskechengziyuanxinxi.objects.filter(q_objects).all()
    count = Discusskechengziyuanxinxi.objects.filter(q_objects).count()
    result = DiscusskechengziyuanxinxiSer(queryset, many=True)
    return Response({'code': 0, 'data': {'list': result.data, 'total': count}})

# 分页
class DiscusskechengziyuanxinxiPage(APIView):
    def get(self, request):
        page = request.query_params.get('page')  # 第几页
        limit = request.query_params.get('limit')  # 数量
        sort = request.query_params.get('sort') # 排序
