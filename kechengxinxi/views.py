...
# 保存
class KechengxinxiSave(APIView):
...
# 修改
class KechengxinxiUpdate(APIView):
    def post(self, request):
        ...
        shhf=data['shhf']
        id = data['id']
        Kechengxinxi.objects.filter(id=id).update(
        id=id,
        kechengmingcheng=kechengmingcheng,
        tupian=tupian,
        shipin=shipin,
...
# 删除
class KechengxinxiDelete(APIView):
    def post(self, request):
        data = request.data
        for id in data:
            Kechengxinxi.objects.filter(id=int(id)).delete()
        return Response({'code': 0, 'msg': '删除成功'})

# 查询详情
class KechengxinxiInfo(APIView):
    def get(self, request, id):
        id = id
        entity = Kechengxinxi.objects.filter(id=int(id)).all()
        entityser = KechengxinxiSer(entity, many=True)
        return Response({'code': 0, 'data': entityser.data[0]})

# 分页
class KechengxinxiList(APIView):
    def get(self, request):
        page = request.query_params.get('page')  # 第几页
        limit = request.query_params.get('limit')  # 数量

        if page is None or limit is None:
            return KechengxinxiListNotPage(request)
        else:
            return KechengxinxiListPage(request)
...
def KechengxinxiListPage(request):
    ...
    query_params = {
                        'id__icontains': request.query_params.get('id'),
                                ...
                                'shipin__icontains': request.query_params.get('shipin'),
    ...
    queryset = Kechengxinxi.objects.filter(q_objects).all()
    count = Kechengxinxi.objects.filter(q_objects).count()
    ...
    result = KechengxinxiSer(items, many=True)
    return Response({'code': 0, 'data': {'list': result.data, 'total': count}})

def KechengxinxiListNotPage(request):
    userid = request.query_params.get('userid')
    query_params = {
                        'id__icontains': request.query_params.get('id'),
                                'addtime__icontains': request.query_params.get('addtime'),
    ...
    queryset = Kechengxinxi.objects.filter(q_objects).all()
    count = Kechengxinxi.objects.filter(q_objects).count()
    result = KechengxinxiSer(queryset, many=True)
    return Response({'code': 0, 'data': {'list': result.data, 'total': count}})

# 分页
class KechengxinxiPage(APIView):
    def get(self, request):
        page = request.query_params.get('page')  # 第几页
        limit = request.query_params.get('limit')  # 数量
        sort = request.query_params.get('sort') # 排序
...
class Recommend(APIView):
    def post(self, request):
        ...
        # 使用 Django ORM
        recommended_items = Kechengxinxi.objects.filter(id__in=keys)
        result = [Kechengxinxi.kechengxinxi_to_dict(item) for item in recommended_items]
